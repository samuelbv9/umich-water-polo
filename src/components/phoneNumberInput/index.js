...
function useHeight() {
	...
	useEffect(() => {
		function updateHeight() {
			setHeight(Math.floor(window.innerHeight / 8) * 8);
		}
...
const List = ({ dropdownClass, anchorEl, onClose, selectedIndex, ...props }) => {
	const listRef = useRef();

	const height = useHeight()
	const menuHeight = .8 * height

	useEffect(() => {
		if (props.itemData.highlightCountryIndex >= 0 && listRef.current) {
			listRef.current.scrollToItem(props.itemData.highlightCountryIndex)
		} else if (listRef.current) {
			listRef.current.scrollToItem(selectedIndex)
		}
	}, [props.itemData.highlightCountryIndex, selectedIndex])
	...
	return (
		<Menu
			...
			PaperProps={{ style: { maxHeight: "calc(100% - 10px)" } }}
		>
			<FixedSizeList ref={listRef} {...props} height={menuHeight}>
				{row}
			</FixedSizeList>
		</Menu>
	)
...
const row = ({ data, index, style }) => {
	const { localization, countries, handleFlagItemClick, isSelected, flags, highlightCountryIndex } = data
	...
	return (
		<div style={style}>
			<Item
				...
				selected={isSelected(country) || index === highlightCountryIndex}
...
class MaterialUiPhoneNumber extends Component {
	...
	constructor(props) {
		...
		const onlyCountries = filteredCountries.map((i, index) => {
			i.index = index
			return i
		})
		...
		let countryGuess;
		if (props.iso2) {
			countryGuess = find(onlyCountries, { iso2: props.iso2 });
		}
		...
		this.state = {
			...
			highlightCountryIndex: countryGuess.index,
			queryString: '',
	...
	handleInput = (e) => {
		...
		const {
			selectedCountry, formattedNumber: oldFormattedText } = this.state;
		const {
		...
		if (e.target.value.length > 0) {
			...
			// if (false && (!freezeSelection || selectedCountry.dialCode.length > inputNumber.length)) {
		...
		this.setState({
			...
			if (onChange) {
				let countryData = this.getCountryData()

				onChange({ iso2: countryData.countryCode, number: formattedNumber, code: countryData.dialCode, format: countryData.format });
			}
		});
	}
	...
	handleFlagItemClick = (country) => {
		const { formattedNumber, onlyCountries } = this.state;
		...
		const newNumber = unformattedNumber // disableCountryCode ? unformattedNumber : (unformattedNumber.length > 1 ? unformattedNumber.replace(currentSelectedCountry.dialCode, nextSelectedCountry.dialCode) : nextSelectedCountry.dialCode);
		...
		this.setState({
			...
			highlightCountryIndex: nextSelectedCountry.index,
			freezeSelection: true,
			...
			if (onChange) {
				let countryData = this.getCountryData()
				onChange({ number: newFormattedNumber, code: countryData.dialCode, format: countryData.format, iso2: countryData.countryCode });
			}
		});
	}
	...
	getHighlightCountryIndex = (direction) => {
		const { highlightCountryIndex: oldHighlightCountryIndex, onlyCountries } = this.state;

		// had to write own function because underscore does not have findIndex. lodash has it
		const highlightCountryIndex = oldHighlightCountryIndex + direction;

		if (highlightCountryIndex < 0 || highlightCountryIndex >= onlyCountries.length) {
	...
	searchCountry = () => {
		const { queryString, onlyCountries } = this.state;

		const probableCandidate = this.getProbableCandidate(queryString) || onlyCountries[0];
		const probableCandidateIndex = findIndex(onlyCountries, probableCandidate);
	...
	handleKeydown = (e) => {
		const {
			anchorEl, highlightCountryIndex, onlyCountries,
	...
	updateFormattedNumber = (number) => {
		...
		if (!inputNumber.startsWith('+')) {
			...
			if (this.props.iso2) {
				countryGuess = find(onlyCountries, { iso2: this.props.iso2 });
			} else {
	...
	getDropdownProps = () => {
		const {
			selectedCountry, anchorEl, onlyCountries, highlightCountryIndex } = this.state;
						...
						: (
							<>
								<Button
									...
									aria-haspopup
								>
									<div style={{ marginLeft: "4px" }} className={inputFlagClasses} /><span style={{ color: "rgba(0, 0, 0, 0.26)", marginLeft: ".25rem", marginRight: "2px" }}>+{selectedCountry.dialCode}</span>
								</Button>


								<List
									selectedIndex={selectedCountry.index}
									anchorEl={anchorEl}
									dropdownClass={dropdownClass}
									onClose={() => this.setState({ anchorEl: null })}
									itemCount={onlyCountries.length}
									itemSize={36}
									width={300}
									itemData={{ flags: this.flags, localization, countries: onlyCountries, handleFlagItemClick: this.handleFlagItemClick, isSelected, highlightCountryIndex }}
								/>
	...
	render() {
		...
		const {
			...
			native, defaultCountry, excludeCountries, onlyCountries,
		...
		const getPlaceholder = () => {
			let counter = 0
			try {
				return selectedCountry.format?.slice(selectedCountry.format?.indexOf(" ") + 1).replaceAll(".", () => {
					counter += 1
					return counter % 10
				}) || "1234567890"
			} catch (error) {
